...
Starts a ``LocalCluster`` of worker processes twice, once with the worker plugin
...
def _measure(path, patched):
    ...
    try:
        ...
        return {"workers_with_pool": sum(1 for v in pools.values() if v[0]),
...
def main():
    ...
    if not path:
        ...
        return
    print(f"file    {path}")
    print(f"cluster {N_WORKERS} worker processes x {THREADS_PER_WORKER} threads, "
          f"{CHUNKS_PER_AXIS}x{CHUNKS_PER_AXIS} chunks")
